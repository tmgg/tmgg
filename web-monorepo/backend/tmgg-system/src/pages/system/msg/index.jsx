...
import {ProTable} from "@tmgg/pro-components";
...
export default class  extends React.Component {
  ...
  read = (record) => {
    HttpUtil.post("/msg/read", record).then(rs => {
      ...
      <Tabs defaultActiveKey="0" destroyInactiveTabPane tabBarExtraContent={<a onClick={()=>this.tableRef.current.reload()}>刷新</a>}>
        <Tabs.TabPane key='0' tab='所有消息'>
          <ProTable
            ...
            request={(params, sort) => HttpUtil.getPageableData("msg/page", params, sort)}
        ...
        <Tabs.TabPane key='1' tab='未读消息'>
          <ProTable
            actionRef={this.tableRef}
            request={(params, sort) => HttpUtil.getPageableData("msg/page?read=false", params, sort)}
            columns={this.columns}
            rowSelection={false}
            rowKey='id'
            search={false}
        ...
        <Tabs.TabPane key='2' tab='已读消息'>
          ...
          <ProTable
            actionRef={this.tableRef}
            request={(params, sort) => HttpUtil.getPageableData("msg/page?read=true", params, sort)}
            rowSelection={false}
            rowKey='id'
            search={false}
            options={false}
