...
class Config {
  ...
  loadLoginInfo() {
    ...
    return new Promise(resolve => {
      HttpClient.get('/getLoginInfo').then(rs => {
        const {data} = rs;
  ...
  public loadSiteInfo = () => {
    return HttpClient.get('site-info').then(rs => {
      let siteInfo = rs.data;
  ...
  public addDataLoadedListener = (listener: any) => {
    this.initDataLoadedListeners.push(listener)
  };
