 ...
 * the License.
 */
package io.tmgg.commons.poi.excel.export.base;

import io.tmgg.commons.poi.cache.ImageCache;
import io.tmgg.commons.poi.entity.BaseTypeConstants;
import io.tmgg.commons.poi.entity.PoiBaseConstants;
import io.tmgg.commons.poi.entity.SpecialSymbolsEntity;
...
public abstract class BaseExportService extends ExportCommonService {
    ...
    public void createImageCell(Cell cell, double height, int rowspan, int colspan,
        ...
        if (type.equals(ExcelType.HSSF)) {
            ...
            anchor = new XSSFClientAnchor(10 * Units.EMU_PER_POINT, 10 * Units.EMU_PER_POINT, 1010 * Units.EMU_PER_POINT, 245 * Units.EMU_PER_POINT, (short) cell.getColumnIndex(), cell.getRow().getRowNum(), (short) (cell.getColumnIndex() + colspan - 1),
                    cell.getRow().getRowNum() + rowspan - 1);
        }
        if (StringUtils.isNotEmpty(imagePath)) {
            data = ImageCache.getImage(imagePath);
