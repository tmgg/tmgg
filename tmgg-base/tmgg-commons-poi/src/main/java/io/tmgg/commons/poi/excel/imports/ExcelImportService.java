 ...
 * the License.
 */
package io.tmgg.commons.poi.excel.imports;
...
import io.tmgg.commons.poi.excel.imports.recursive.ExcelImportForkJoinWork;
...
public class ExcelImportService extends ImportBaseService {
        ...
        if (params.getImportFields() != null) {
            ...
            if (params.isNeedCheckOrder()) {
                ...
                for (Integer key : titlemap.keySet()) {
                    if (!StringUtils.equals(titlemap.get(key), params.getImportFields()[key])) {
                        LOGGER.error("excel列顺序不一致");
                        throw new ExcelImportException(ExcelImportEnum.IS_NOT_A_VALID_TEMPLATE);
                    }
                }
    ...
    private void saveImage(Object object, String picId, Map<String, ExcelImportEntity> excelParams,
        ...
        if (params.getPoiFileHandler() != null){
            setValues(excelParams.get(titleString), object,
                    params.getPoiFileHandler().saveImgFile(fileName, data, object));
        } else if (excelParams.get(titleString).getSaveType() == 1) {
