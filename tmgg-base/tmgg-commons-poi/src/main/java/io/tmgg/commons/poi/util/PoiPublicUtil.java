...
import io.tmgg.commons.poi.cache.ImageCache;
import io.tmgg.commons.poi.entity.ImageEntity;
import io.tmgg.commons.poi.excel.annotation.Excel;
import io.tmgg.commons.poi.excel.annotation.ExcelCollection;
import io.tmgg.commons.poi.excel.annotation.ExcelEntity;
import io.tmgg.commons.poi.excel.annotation.ExcelIgnore;
...
import static io.tmgg.commons.poi.util.PoiElUtil.END_STR;
import static io.tmgg.commons.poi.util.PoiElUtil.START_STR;
...
public final class PoiPublicUtil {
    ...
    public static Object getRealValue(String currentText, Map<String, Object> map) throws Exception {
        ...
        while (currentText.indexOf(START_STR) != -1) {
            ...
            //判断图片或者是集合
            if (obj instanceof ImageEntity || obj instanceof List || obj instanceof ExcelListEntity || obj instanceof WordTable) {
                return obj;
            } else if (obj != null) {
    ...
    public static Object getValueDoWhile(Object object, String[] paramsArr, int index) throws Exception {
        ...
        if (object instanceof Map) {
            ...
            object = PoiReflectorUtil.fromCache(object.getClass()).getValue(object, params);
        }
    ...
    public static List<String> autoSplit(String content) {
        String[] contentArr = content.split("\r\n");
        List<String> list = new ArrayList<>();
        list.addAll(Arrays.asList(contentArr));
        for (int i = list.size() - 1; i >= 0; i--) {
            String[] arr = list.get(i).split("\n");
            if (arr.length > 1) {
                list.remove(i);
                list.addAll(i, Arrays.asList(arr));
            }
        }
        for (int i = list.size() - 1; i >= 0; i--) {
            String[] arr = list.get(i).split("\r");
            if (arr.length > 1) {
                list.remove(i);
                list.addAll(i, Arrays.asList(arr));
            }
