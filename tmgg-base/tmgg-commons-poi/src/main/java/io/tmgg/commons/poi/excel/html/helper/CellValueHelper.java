package io.tmgg.commons.poi.excel.html.helper;
...
public class CellValueHelper {
    ...
    public String getHtmlValue(Cell cell) {
        if (CellType.BOOLEAN == cell.getCellType()
            ...
            if (cell.getRichStringCellValue().numFormattingRuns() == 0) {
                return XmlUtil.escape(cell.getStringCellValue());
            } else if (is07) {
    ...
    private String getHSSFRichString(HSSFRichTextString rich) {
        ...
        for (int i = 0; i < nums; i++) {
            ...
            if (i < nums - 1) {
                sb.append(XmlUtil.escape(text.substring(currentIndex, rich.getIndexOfFormattingRun(i + 1))));
            } else {
                sb.append(XmlUtil.escape(text.substring(currentIndex, text.length())));
            }
            sb.append("</span>");
        }
        return sb.toString();
    ...
    private String getXSSFRichString(XSSFRichTextString rich) {
        ...
        for (int i = 1; i <= nums; i++) {
            ...
            sb.append(
                    XmlUtil.escape(text.substring(lastIndex, currentIndex)));
            sb.append("</span>");
